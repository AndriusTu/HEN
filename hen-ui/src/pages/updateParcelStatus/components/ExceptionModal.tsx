...
function ExceptionModal(state: any) {
  const [transferObject, setTransferObject] = useState(
    state.transferObject as StatusUpdateModel,
  );
  const [responseVersion, setResponseVersion] = useState(
    state.modalResponseVersion,
  );
  ...
  const getLastState = async () => {
    ...
    const data = await getParcelById(parcelId).then((responseData: Parcel) => {
      const latestDate = new Date(
        Math.max(
          ...responseData.parcelStatus.map((e) =>
                ...
                <div className="sm:flex sm:items-start">
                  ...
                  <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                    <h3
                      ...
                      id="modal-title"
                    >
                      Status update has been interrupted
                    </h3>
                    <p className="text-sm text-gray-500">
                      Error occurred while trying to submit your parcel status
                      update. You can either try again or cancel current status
                      update submission
                    </p>
                    {compare ? (
                      <div className="sm:flex-col content-stretch">
                        <div className="mt-2 w-[46%] p-[1%] inline-block align-text-top pt-0">
                          <h2 className="text-sm text-gray-500">
                            State you tried to submit:{' '}
                          </h2>
                          <p>
                            {
                              statusOptions.filter(
                                (option) =>
                                  option.value === state.transferObject.status,
                              )[0].label
                            }
                          </p>
                          <p className="text-sm text-gray-500">
                        ...
                        <div className="mt-2 w-[46%] inline-block align-text-top border-l-2 pl-3 pt-0">
                          ...
                          <p>
                            ...
                            {
                              statusOptions.filter(
                                (option) => option.value === lastState.status,
                              )[0].label
                            }
                          </p>
                          <p className="text-sm text-gray-500">
                            Database parcel version:
                          </p>
                          <p> {responseVersion} </p>
                        </div>
                      </div>
